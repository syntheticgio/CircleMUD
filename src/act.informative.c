...
/*
 * Return health, mana, and movement / stamina to player
 */

ACMD(do_health_mana) {
  // TODO: Add more informative message here about health
  send_to_char(ch, "Health:[%s%d/%d+%d%s]  Mana:[%s%d/%d+%d%s]  Move:[%s%d/%d+%d%s]\r\n",
               CCGRN(ch, C_NRM), GET_HIT(ch), GET_MAX_HIT(ch), hit_gain(ch), CCNRM(ch, C_NRM),
               CCGRN(ch, C_NRM), GET_MANA(ch), GET_MAX_MANA(ch), mana_gain(ch), CCNRM(ch, C_NRM),
               CCGRN(ch, C_NRM), GET_MOVE(ch), GET_MAX_MOVE(ch), move_gain(ch), CCNRM(ch, C_NRM));
}
