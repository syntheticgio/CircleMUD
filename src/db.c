...
void index_boot(int mode) {
    ...
    /*
     * Allocation of space for global tables...
    ...
    switch (mode) {
        ...
        case DB_BOOT_ZON:
            ...
            // do {
            //    if ( (rec_count) * sizeof(zone_data) <= 0 )
            //        log("SYSERR: Zero bytes or less requested at %s:%d.", __FILE__, __LINE__);
            //    if ( !( (zone_table) = (zone_data *) calloc( (rec_count), sizeof(zone_data) ) ) )
            //         {
            //            perror("SYSERR: malloc failure"); abort();
            //         }
            // } while(0)
...
void parse_mobile(FILE *mob_f, int nr) {
    ...
    if (MOB_FLAGGED(mob_proto + i, MOB_AGGRESSIVE) &&
        MOB_FLAGGED(mob_proto + i, MOB_AGGR_GOOD | MOB_AGGR_EVIL | MOB_AGGR_NEUTRAL))
...
// TODO: This is where the mobile is created from mob files - make this object vs structure?
// TODO: is this a specific instance or a prototype?
// TODO: How to add this as an object instead of strucutre - remake structure for char_data as class?
...
void char_to_store(struct char_data *ch, struct char_file_u *st) {
    ...
    if (ch->player.description) {
        if (strlen(ch->player.description) >= sizeof(st->description)) {
            log("SYSERR: char_to_store: %s's description length: %zd, max: %zd! "
 ...
 */
// TODO: Add to character creation
...
int check_bitvector_names(bitvector_t bits, size_t namecount, const char *whatami, const char *whatbits) {
    ...
    for (flagnum = namecount; flagnum < sizeof(bitvector_t) * 8; flagnum++)
        if ((1 << flagnum) & bits) {
            log("SYSERR: %s has unknown %s flag, bit %d (0 through %zd known).", whatami, whatbits, flagnum,
                (namecount - 1));
            error = TRUE;
        }
